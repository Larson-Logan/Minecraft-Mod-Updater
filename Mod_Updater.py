...
def download_mod(download_url, save_path):
...
def get_download_url_modrinth(mod_name, mod_loader, mc_version):
    ...
    try:
        ...
        if not results:
            return None, "No matching mod found."
        ...
        for version in versions:
            ...
            if mod_loader in version["loaders"] and mc_version in version["game_versions"]:
                return version["files"][0]["url"], None
    ...
    try:
        ...
        for index, mod in enumerate(mod_list):
            if "name" in mod and "url" in mod and is_valid_url(mod["url"]):
                mod_name = mod["name"]
                mod_url, error = get_download_url_modrinth(mod_name, mod_loader, mc_version)
                if not mod_url:
                    log_message(f"Failed to resolve URL for {mod_name}: {error}")
                    failed_downloads.append({"name": mod_name, "reason": error})
                    progress_bar["value"] = index
                    continue
                save_path = os.path.join(save_directory, f"{mod_name}_{mod_loader}.jar")
                try:
                    log_message(f"Downloading {mod_name}...")
                    download_mod(mod_url, save_path)
...
def start_download_threaded(json_entry, save_entry, mod_loader_box, mc_version_entry, log_area, progress_bar):
    try:
        thread = threading.Thread(
            ...
            args=(json_entry.get(), save_entry.get(), mod_loader_box.get(), mc_version_entry.get(), log_area, progress_bar),
            daemon=True
        )
        thread.start()
        thread.join()  # Ensure exceptions within the thread are logged properly
    except Exception as e:
...
def create_gui():
    ...
    save_entry = create_entry(root, 50, 1, 1, padx=10, pady=5)
    ...
    mc_version_entry = create_entry(root, 50, 3, 1, padx=10, pady=5)
    mc_version_entry.insert(0, "1.21.1")
